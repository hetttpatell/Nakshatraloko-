...
const ProductsPage = () => {
  ...
  const optionsfilter = useMemo(() => {
    if (!allProducts || allProducts.length === 0)
      return { Ratings: [], Categories: [], Price: [] };
    ...
    return {
      ...
      Price: Array.from(priceRangesSet).sort(
        ...
        {Array.from({ length: 5 }).map((_, i) => {
          if (i < fullStars)
            return (
              <Star
                key={i}
                className="w-3 h-3 md:w-4 md:h-4 text-[var(--color-rating)] fill-[var(--color-rating)]"
              />
            );
      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        <div className="text-center mb-6 md:mb-8">
          <h1 className="text-2xl md:text-4xl lg:text-5xl font-playfair font-bold text-[var(--color-text)] mb-3 md:mb-4">
            Exquisite{" "}
            <span className="text-[var(--color-primary)]">Gemstone</span>{" "}
            Collection
          </h1>
          <p className="text-sm md:text-base text-[var(--color-text-light)] max-w-2xl mx-auto px-2">
            Discover our handcrafted gemstone jewelry, meticulously designed to
            enhance your spiritual journey and personal elegance.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 p-4 md:p-6 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow">
          <div className="flex items-center gap-4 relative w-full md:w-auto">
            <button
              ...
              className="flex items-center gap-2 px-4 md:px-6 py-2 md:py-3 bg-[var(--color-primary-light)] text-[var(--color-primary)] rounded-xl hover:bg-[var(--color-primary)] hover:text-white transition-all w-full md:w-auto justify-center"
            >
            ...
            {hasActiveFilters && (
              <button
                ...
                className="hidden md:flex items-center gap-2 px-4 py-2 text-[var(--color-text-light)] hover:text-[var(--color-primary)] transition-colors"
              >
                <X size={16} /> Clear All
              </button>
            )}
              ...
              <div className="mt-2 bg-white border border-[var(--color-border)] rounded-xl shadow-lg py-2">
                {sortOptions.map((o) => (
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 hover:bg-[var(--color-primary-light)] ${
                      sortBy === o.value
        ...
        <div className="pb-12 md:pb-16">
          {filteredAndSortedProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 md:py-20">
              ...
              <p className="text-sm md:text-base text-[var(--color-text-light)] mb-6 text-center">
                Try adjusting your filters to see more results.
              </p>
              <button
                onClick={resetFilters}
                className="px-5 py-2.5 md:px-6 md:py-3 bg-[var(--color-primary)] text-white rounded-xl hover:bg-[var(--color-primary-dark)] transition-colors text-sm md:text-base"
              >
                Reset Filters
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
              {filteredAndSortedProducts.map((product) => (
                <Link
                  ...
                  state={{ product }}
                  className="group bg-[var(--color-surface)] rounded-xl md:rounded-2xl overflow-hidden border border-[var(--color-border)] transition-all duration-300 hover:shadow-md md:hover:shadow-xl hover:-translate-y-0.5 md:hover:-translate-y-1"
                >
                  ...
                  <div className="p-3 md:p-4 lg:p-5">
                    ...
                    <h3 className="font-semibold text-[var(--color-text)] text-sm md:text-base mb-2 line-clamp-2 group-hover:text-[var(--color-primary)] transition-colors">
                      {product.name}
                    </h3>
                    ...
                    <div className="flex flex-col gap-1.5 md:gap-2 pt-2 md:pt-3 border-t border-[var(--color-border-light)]">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button className="flex items-center gap-1 text-[var(--color-primary)] hover:text-[var(--color-primary-dark)] font-medium text-xs md:text-sm transition-colors">
                          View{" "}
                          <ArrowRight className="w-3 h-3 md:w-4 md:h-4 transform group-hover:translate-x-0.5 md:group-hover:translate-x-1 transition-transform" />
                        </button>
                      </div>
                    </div>
                  </div>
