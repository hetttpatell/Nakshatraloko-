      ...
      .then((res) => {
        if (res.data.success) {
          ...
          const transformedData = res.data.data.map((category, index) => ({
            ...
            img: category.Image || "/abot.jpg", // Use default image if none provided
            path: `/category/${category.CategoryName.toLowerCase().replace(/\s+/g, '-')}`,
            items: category.items ||`${Math.floor(Math.random() * 20) + 15} Products`, // Generate random product count
