...
export default function Collections() {
  ...
  useEffect(() => {
    axios.post(`${BACKEND_URL}getAllFeaturedCategory`).then((res) => {
      if (res.data.success) {
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-12 md:mb-16 lg:mb-20">
          ...
          <motion.p
            ...
            viewport={{ once: true }}
          >
            Explore our handcrafted collections designed to enhance your
            spiritual journey and personal style.
          </motion.p>
        </div>
            ...
            >
              <Link to={collection.path} className="block h-full ">
                ...
                <div className="relative h-68 sm:h-32 md:h-40 lg:h-48 xl:h-150 overflow-hidden">
                  ...
                  {/* Gradient overlay */}
                  <div
                    className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent opacity-80 
                ...
                >
                  <h3 className="text-sm md:text-base lg:text-lg font-bold mb-1 md:mb-1.5 line-clamp-1">
                    {collection.name}
                  </h3>
                  <p className="text-xs md:text-sm text-[var(--color-primary-light)]/90 mb-2 md:mb-3">
                    {collection.items}
                  </p>
                ...
                {/* Color accent bar */}
                <div
                  className={`absolute bottom-0 left-0 right-0 h-1 ${collection.color} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                ></div>
              </Link>
            </motion.div>
          ))}
