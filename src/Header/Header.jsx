...
import { ShoppingCart, Heart, User, ChevronDown, ChevronRight, UserCog } from "lucide-react";
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          `flex items-center gap-1 px-3 py-2 transition-colors duration-300 relative rounded ${
            isActive || isParentActive ? "text-orange-400" : "text-white group-hover:text-orange-400"
          }`
        }
      >
        {item.label}
    ...
    <>
      <header className="sticky top-0 z-50 bg-[var(--color-navy)]/95 backdrop-blur-md shadow-md border-b border-white/10">
        <nav className="relative w-full py-4 px-6 lg:px-12 flex items-center">
          {/* Logo */}
          <NavLink to="/" className="flex-shrink-0 cursor-pointer" onClick={closeMenu}>
            <img src={logo} alt="Nakshatraloko Logo" className="w-20 sm:w-22" />
          </NavLink>
            ...
            {isLoggedIn ? (
              <>
                ...
                {isAdmin && (
                  <NavLink
                    to="/admin"
                    className="relative p-2 rounded-full transition-colors duration-300 hover:text-orange-400"
                  >
                    <UserCog  className="h-6 w-6 stroke-[1.5]" /> 
                  </NavLink>
                )}
              </>
            ) : (
          ...
          <div className="lg:hidden flex items-center space-x-4 relative z-20 ml-auto">
            ...
            {/* Mobile Cart/Wishlist/Admin */}
            {isLoggedIn && (cartCount > 0 || wishlistCount > 0 || isAdmin) && (
              <div className="flex items-center space-x-3">
                {cartCount > 0 && (
                  <NavLink to="/cart" className="relative p-2 rounded-full text-white">
                    <ShoppingCart className="h-6 w-6 stroke-[1.5]" />
                    {cartCount > 0 && <Badge count={cartCount} />}
                  </NavLink>
                )}
                {wishlistCount > 0 && (
                  <NavLink to="/wishlist" className="relative p-2 rounded-full text-white">
                ...
                {isAdmin && (
                  <NavLink
                    to="/admin"
                    className="relative p-2 rounded-full text-white hover:text-orange-400"
                  >
                    <UserCog className="h-6 w-6 stroke-[1.5]" /> 
