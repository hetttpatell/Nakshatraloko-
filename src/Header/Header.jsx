  ...
  return (
    <li className="relative group" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <NavLink
        to={item.to}
        className={({ isActive }) =>
          `flex items-center gap-1 px-3 py-2 transition-colors duration-300 relative rounded ${isActive || isParentActive
            ? "text-orange-400"
      ...
      >
        ...
        {item.subMenu && (
          <ChevronDown
            className={`w-4 h-4 transition-transform duration-300 ${dropdownOpen ? "rotate-180 text-orange-400" : "rotate-0"
              }`}
          />
        )}
      ...
      {item.subMenu && (
        <ul
          className={`absolute left-0 mt-2 bg-white rounded shadow-lg min-w-[180px] transform transition-all duration-300 ${dropdownOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 pointer-events-none"
            }`}
        >
          {item.subMenu.map((sub) => (
            <li key={sub.label}>
              <NavLink
                to={sub.to}
                className={({ isActive }) =>
                  `block px-5 py-2 text-sm font-normal ${isActive ? "bg-orange-100 text-orange-600" : "text-gray-900 hover:bg-orange-100"
                  }`
                }
              >
                {sub.label}
  ...
  >
    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      />
          ...
          {/* Mobile Menu */}
          <div
            className={`lg:hidden fixed top-[64px] left-0 w-full bg-[var(--color-navy)]/95 backdrop-blur-md shadow-md transform transition-all duration-300 ease-in-out ${menuOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0 pointer-events-none"
              }`}
          >
                  ...
                  {item.subMenu ? (
                    <>
                      ...
                      >
                        {item.label}
                        <ChevronRight
                          className={`w-4 h-4 transition-transform duration-300 ${openSubMenu === item.label ? "rotate-90 text-orange-400" : ""
                            }`}
                        />
                      </button>
                      <ul
                        className={`overflow-hidden transition-all duration-300 ${openSubMenu === item.label ? "max-h-40 mt-2" : "max-h-0"
                          }`}
                      >
                        {item.subMenu.map((sub) => (
                          <li key={sub.label}>
